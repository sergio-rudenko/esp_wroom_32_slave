...
- ожидание новых настроек в no-link режиме выполняется короткими срезами (`500ms`) с регулярным feed WDT (без блокирующего ожидания на `15s`).
- операции DNS/UDP sync выполняются в блокирующем режиме, поэтому на время `sync_once(...)` задача временно отписывается от TWDT и подписывается обратно сразу после вызова.
